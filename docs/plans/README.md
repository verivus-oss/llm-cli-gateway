...
36 `*.dag.toml` implication maps plus supporting drafts, prompts and workflows.
...
| `postgres-worktree-cleanup-durability.dag.toml` | PostgreSQL session deletion and worktree cleanup | DONE 2026-09-04, issue #305, all seven steps. Closes the gap #302 documented: PostgreSQL deletion now stages a caller-hidden cleanup tombstone and retains a failed removal for owning-host retry. Read it before touching session deletion, `cleanup_expired_sessions`, or the worktree cleanup observer. Gated by `npm run session:tombstone:scope:check`. Residuals are recorded in the plan, not closed: a tombstone is unbounded. |
