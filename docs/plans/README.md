...
| `postgres-worktree-cleanup-durability.dag.toml` | PostgreSQL session deletion and worktree cleanup | IN PROGRESS, issue #305. Closes the gap #302 documented: PostgreSQL deletion did not retain a failed worktree removal for owning-host retry. Step `hide-tombstones-from-reads` is done and gated by `npm run session:tombstone:scope:check`; the staging half is not written yet. |
