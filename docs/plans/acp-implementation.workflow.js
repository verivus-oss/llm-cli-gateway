...
function implementPrompt(stepId, phase) {
  return [
    ...
    ``,
    `Then: run \`npm run build\` and the relevant \`npm test\` suites. Commit on the current feature branch with a precise message (no Co-Authored-By trailer).`,
...
function reviewPrompt(reviewer, phase, packet) {
  ...
  return [
    ...
    `Return APPROVED only if ALL hold, each backed by something you personally inspected or executed: every report claim verified against actual code/tests/docs; the DAG validation clause and [test_matrix] rows satisfied by real non-vacuous tests; existing CLI transport path unchanged; every applicable [security_invariants] entry holds (no raw ACP JSON-RPC / prompt text / file contents / credential paths reaching logs, flight recorder, doctor, or resources; no shell eval for entrypoints; deny-by-default HostServices; ApprovalManager-mediated permissions); no console.log/stdout writes from gateway code.`,
    `Do NOT approve on intent, plan-compliance, "looks correct", or "should be fixed later". For anything wrong return CHANGES_REQUIRED with file:line + what is wrong + inspected evidence. For an unfixable in-scope defect return BLOCKER with evidence.`,
    `"""`,
    ``,
    ...
    `Translate the reviewer's reply into the structured schema. reviewer="${reviewer}".`,
  ].join('\n')
}
...
function fixPrompt(phase, findings, packet) {
  return [
    ...
    `For each finding:`,
    `- If it is correct: fix it in code/tests, re-run \`npm run build\` + relevant \`npm test\` + \`npm run upstream:contracts\`, and commit. Add the test name to addressed[].`,
...
async function runStep(stepId, phase) {
  const impl = await agent(implementPrompt(stepId, phase), {
  ...
  const ver = await agent(verifyPrompt(stepId, phase), {
...
async function reviewGate(phase, packet) {
  ...
  while (round <= MAX_REVIEW_ROUNDS) {
    ...
    const reviews = (await parallel(phase.lenses.map((lens) => () =>
      agent(reviewPrompt(lens, phase, currentPacket), {
    ...
    const fix = await agent(fixPrompt(phase, findings, currentPacket), {
...
// ---------------------------------------------------------------------------
log('ACP implementation workflow starting — feature branch + per-phase review gate; stops at release-ready, never publishes.')

await agent(
  `Bootstrap: create the feature branch off master for the ACP slice (e.g. feat/acp-gateway-extension) if not present; create ${STATE_DIR}, ${VERIFY_DIR}, ${REVIEW_DIR}. Resolve every [open_questions] entry in ${DAG} using the documented defaults (process reuse = per gateway-session w/ idle reaping; default_transport stays cli; adapters user-installed/deferred) and write them to ${STATE_DIR}/decisions.md. Return "ok".`,
  { label: 'bootstrap', phase: PHASES[0].title })
...
const allApproved = summary.length === PHASES.length && summary.every((s) => s.approved)
log(allApproved
