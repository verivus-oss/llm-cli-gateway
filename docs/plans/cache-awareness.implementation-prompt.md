...
3. **Sync MCP responses don't include the model output in the tool result** — they return only metadata `{cli, correlationId, durationMs, exitCode}`. The actual response is in the flight recorder. Read it back through the gateway, never through the database:
    ...
    Do not open `logs.db` or connect to PostgreSQL. `[persistence].backend` selects the durable engine, and `backend = "postgres"` puts request history in PostgreSQL with the other durable subsystems. A backend change does not migrate or cross-read old rows. Use the gateway tools so the current engine is selected correctly and the ownership checks remain in force.
