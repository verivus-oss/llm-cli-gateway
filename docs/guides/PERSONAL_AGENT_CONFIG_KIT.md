...
For Mistral, Kit uses a third isolation model, a controlled environment, because Vibe has neither a bare flag nor a debug surface the gateway could inspect the effective prompt through. Instead of scrubbing individual levers and verifying the result, the gateway constructs the complete environment Vibe reads. `HOME` and `VIBE_HOME` are redirected into a fresh per-attempt directory that the gateway creates and whose exact file manifest it asserts before launch, so `~/.agents`, `~/.vibe`, and every other home-relative default resolve inside gateway-owned space; the assertion fails closed if any ambient surface (a skills, agents, or tools directory) is present. The gateway writes the only configuration file in that home: a match-nothing skill allowlist plus a `[session_logging]` block. It forces `include_project_context`, `include_prompt_detail`, and the experimental registry-skills switch off through the `VIBE_*` environment layer, and separately scrubs the bare-name environment variables (such as `SAVE_DIR` and `ENABLED`) that Vibe's nested settings models read without a `VIBE_` prefix, which that layer cannot reach. The selected canonical folder is left untrusted, so a repository's own committed `.vibe/config.toml` or `hooks.toml` is inert. The managed agent posture is forced regardless of caller input, and the compiled context rides as a stamped prompt prefix whose digest is bound to the isolation plan, so context drift fails closed rather than running with the wrong instructions.

Because that redirected home has no OS keyring and the gateway disables keyring access, Mistral Kit calls require `MISTRAL_API_KEY` in the gateway process environment. A working interactive `vibe` login on the workstation is not a readiness check for a Kit request. Conversely, the ambient `~/.vibe/config.toml` is never read on a Kit turn, so an `[session_logging] enabled = false` there neither blocks Kit native continuity nor is required to be corrected for it; that setting affects non-Kit `mistral_request` `--continue` / `--resume` only. `doctor` reports both facts: `personal_config.provider_eligibility.mistral` carries Kit eligibility and its credential blocker, and `client_config.vibe_session_logging.kit_note` states the Kit's independence from that file.
...
On a successful first Claude run, the gateway supplies a fresh UUID to Claude and may resume by explicit native session ID while that gateway process remains alive. Codex may do the same only when its structured output provides a verified native session ID. Mistral does the same from disk: the per-attempt home is ephemeral, but the Kit points Vibe's `[session_logging] save_dir` at a stable gateway-owned `0700` directory keyed by the Kit execution identity, so the gateway can read the native session ID that turn actually wrote and later pass it to `--resume`. A configuration change produces a different execution identity and therefore a fresh directory and conversation. The gateway never uses a provider's process-global "latest session" selector in Kit mode. A gateway restart intentionally retires every Kit native continuation and requires a fresh native conversation.

Vibe mints native session IDs in the 8-4-4-4-12 hexadecimal shape but does not fill in the RFC 4122 version and variant nibbles, so most of them fail the strict UUID guard the other providers use. The Kit therefore applies a Vibe-scoped broad guard that accepts the lowercase-hex 8-4-4-4-12 shape for Mistral only; Claude and Codex keep the strict guard. That relaxation is safe because the value is charset-gated (it can carry no shell or path metacharacter) and because the resolver additionally requires an exact matching session ID on disk before anything is passed to `--resume`.

Native handles are local provider state. A second workstation gets the same verified baseline and repository instructions after `config_sync()`, but it starts its own native conversation. This prevents two workstations working in the same repository from accidentally resuming or overwriting each other's provider state. Sibling working folders also have distinct Kit execution identities, so their Codex project-root controls, Mistral session directories, and native handles cannot be crossed.
...
- To check whether a given provider can run a Kit turn on this workstation, read `config_status().kitProviders.<provider>` for baseline and credential state, or `doctor`'s `personal_config.provider_eligibility.<provider>` for the same plus durable-admission state and an explicit blocker list. Both report only whether a required credential environment variable is present, never its value. `provider_tool_capabilities` carries the same support fact and isolation model per provider.
