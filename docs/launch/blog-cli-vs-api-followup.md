...
Codex shipped `exec resume <session-id>` and `exec resume --last`, and the gateway now wires both. (Since publication: do not rely on which session `--last` selects or on the resumed working directory; both are under review.) If you pass a real Codex session UUID (the kind that lives in `~/.codex/sessions/`), `codex_request` invokes `exec resume` and you get genuine continuity, the same tool-use history, file context, and partial work the CLI itself preserves. `resumeLatest: true` resumes without you having to look the UUID up, though see the caveat above about which session that is.
