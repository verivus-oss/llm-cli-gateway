...
- Stdio is the unrestricted local development transport for machine-local filesystem paths. `[workspaces].allow_unregistered_working_dir` is an inert legacy key that nothing reads; it is neither a local control nor an HTTP or tunnel bypass, and setting it warns at startup.
