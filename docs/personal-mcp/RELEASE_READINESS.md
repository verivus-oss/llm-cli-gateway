...
| node:sqlite is confined to the adapter; no dynamic PRAGMA helper    | `scripts/release-security-audit.sh` scans non-test `src/` files: `node:sqlite` may be referenced only by `src/sqlite-driver.ts`, and no `.pragma()` helper call may appear anywhere; fixed SQLite setup uses literal `PRAGMA` SQL through `exec` |
...
| Gate                                                                                              | Evidence                                                                                                                      |
| ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Mechanical argv/env behaviour for registered provider CLIs has one source of truth                | `src/upstream-contracts.ts` (`UPSTREAM_CLI_CONTRACTS`) + `validateUpstreamCliArgs` / `validateUpstreamCliEnv`                 |
