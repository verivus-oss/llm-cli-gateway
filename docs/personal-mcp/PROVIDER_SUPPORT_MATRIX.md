...
| Mistral (Vibe)                | Inbound CLI MCP host and outbound validation provider     | Vibe reads MCP server configuration from `VIBE_HOME` config; gateway calls Vibe outbound (`mistral_request`/`mistral_request_async`)                                  | Verified CLI and outbound         | Local `vibe --help` and gateway `doctor --json` provider block                                          |
...
Vibe is Mistral's coding CLI. It reads its MCP server configuration from `VIBE_HOME` config; `vibe --help` does not advertise a `vibe mcp` subcommand. A locally running gateway can therefore be registered through that provider configuration for inbound stdio use. Outbound, the gateway drives Vibe headlessly through `mistral_request` / `mistral_request_async` (programmatic mode with `--trust`, `--max-turns`, `--max-price`, `--max-tokens` caps); the provider block in `llm-cli-gateway doctor --json` reports the installed Vibe version, active model, and session-logging state.
