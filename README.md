...
`session_delete` removes gateway tracking from caller-visible session surfaces. With either session manager, it also invokes best-effort cleanup for associated gateway-owned lifecycle resources, such as a managed worktree, on the host processing deletion. A durably owned worktree session remains as a hidden cleanup-pending tombstone until Git removal is verified; failed removal is retried when a manager is registered on the owning host. It does not delete a provider's own stored conversation.

Cleanup is the owning host's, under either engine. Deletion processed from
another host removes no worktree and leaves the owning host's record intact,
rather than dropping tracking. `session_clear_all` has the same per-session
behavior. The database-side `cleanup_expired_sessions` function stages the same
tombstone instead of deleting a worktree-bearing session; it invokes no gateway
observer and so attempts no removal itself. A tombstone is retained until
cleanup succeeds and is not bounded by retention.
