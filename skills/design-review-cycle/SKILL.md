...
- For multi-round design reviews where the design document itself doesn't move much between rounds, switch from `prompt` to the structured `promptParts` field: put the stable review brief in `system`, the design content (or path-to-design) in `context`, and the round-specific question in `task`. `prompt` and `promptParts` are mutually exclusive. Stable `system`/`context` across rounds keeps the prefix-hash identical, raising implicit cache hit rate at the provider. After a round, read `cache_state://prefix/{hash}` to confirm reviewers actually shared the prefix.
