...
export const PROVIDER_TARGET_VERSIONS: Record<CliType, string> = {
  ...
  // hash out of that banner regardless of the marker. A hash-less target
  // therefore compares "1.0.4" against "1.0.4 (d846eb93d9)" and can never
  // match. `providers:rebaseline --apply` first wrote exactly that bare
  // `grok 1.0.4`, because `normalizeProviderVersion` only accepted a build id
    ...
    adminSubcommands: [
      {
        ...
        evidence:
          "2.1.233 (Claude Code) root help advertises `import` (re-probed 2026-08-18: `claude import --help` returns `Import config from another AI coding agent into Claude Code`). Auto-declared by `npm run providers:rebaseline`. safety is the conservative default: a read-only misclassification would remove a control, a gated one only adds an approval, so generated families are never `read-only`. UNVERIFIED pending maintainer review.",
      },
  ...
  grok: {
    ...
    discovery: {
      ...
      modelDiscovery: {
        ...
        facts: {
          // grok 1.0.4 --help declares `--reasoning-effort <EFFORT>` with
          // `[aliases: --effort]` and NO possible-values line. An earlier
          // comment here cited a `[possible values: low, medium, high, xhigh,
          // max]` line that the binary has never printed; it was a fabricated
          // citation, not an observation. These five are the levels the gateway
          // ADVERTISES for discovery, not a set the binary enforces: the
          // contract deliberately carries no `values` for either spelling, so
          // any level the caller passes reaches grok. Re-probed 2026-08-18.
          effortLevels: ["low", "medium", "high", "xhigh", "max"],
          supportsFallbackModelChain: false,
          aliases: [],
          agentProfiles: [],
    ...
    adminSubcommands: [
      {
        ...
        safety: "mutating-gated",
        evidence:
          "grok 1.0.3 (1a29d5bc12) root help advertises `du`. Auto-declared by `npm run providers:rebaseline`. safety is the conservative default: a read-only misclassification would remove a control, a gated one only adds an approval, so generated families are never `read-only`. STILL PRESENT at grok 1.0.4 (d846eb93d9), which now describes it: `du  Show what the grok home (~/.grok) uses on disk [aliases: disk-usage]`. That description reads read-only and makes this a reclassification candidate, but it is NOT reclassified here: help text says what a command is for, not what it writes, and loosening a safety class removes a control, which is not a rebaseline's job. Still UNVERIFIED pending maintainer review.",
      },
