...
describe("the session DATABASE_URL must be a PostgreSQL URI, not merely a URL", () => {
  // `DatabaseUrlSchema` requires the raw PostgreSQL authority prefix.
  ...
  // A bare string schema accepts `http://`, `ftp://` and `file://`, and every
  // one reaches `redactDsn` through `roleDsns.app` on the doctor path.
  //
  ...
  ])("refuses %s outside the PostgreSQL category", (_name, databaseUrl) => {
