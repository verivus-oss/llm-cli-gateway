  ...
  describe("defaultSpawn (real process)", () => {
    ...
    it.skipIf(process.platform === "win32")(
      ...
      async () => {
        ...
        // Pin the stub's module type. Node infers it for an extensionless file
        // by walking up for the nearest package.json, which escapes the temp
        // dir and lands on whatever sits above tmpdir(). A stray
        // `"type": "module"` there loads this CommonJS stub as ESM, `require`
        // is undefined, the stub dies before writing its pid file, and the test
        // fails with an unrelated ENOENT. That is exactly what happened on the
        // self-hosted runner, whose TMPDIR is /tmp, while a developer TMPDIR
        // elsewhere passed by luck.
