...
describe("acp provider registry", () => {
  ...
  it("targets the single-source Antigravity agy version for gemini and keeps it on the watchlist with no entrypoint", () => {
    const gemini = getAcpProviderEntry("gemini");
    expect(gemini.displayName).toBe("Google Antigravity");
    expect(gemini.targetVersion).toBe(PROVIDER_TARGET_VERSIONS.gemini);
