...
async function startSleeper(
  ...
  return (await manager.startJob("sleep" as LlmCli, [String(seconds)], corr)).id;
}
...
describe("AsyncJobManager limiter (issue #130)", () => {
  ...
  it("queues a process job when the global running limit is saturated", async () => {
    const manager = makeManager({ maxRunningJobs: 1, maxRunningJobsPerProvider: 5 });
    const a = await manager.startJob("sleep" as LlmCli, ["2"], "corr-a");
    const b = await manager.startJob("sleep" as LlmCli, ["2"], "corr-b");

    expect((await manager.getJobSnapshot(a.id)!).status).toBe("running");
    expect((await manager.getJobSnapshot(b.id)!).status).toBe("queued");
    ...
    await manager.cancelJob(b.id);
  });

  it("bounds a burst of async process jobs to configured running and queue caps", async () => {
    ...
    for (let i = 0; i < 12; i++) {
      jobs.push((await manager.startJob("sleep" as LlmCli, ["2"], `corr-burst-${i}`)).id);
      const snap = manager.getLimiterSnapshot();
    ...
    for (const job of jobs) await manager.cancelJob(job);
  });
  ...
  it("per-provider limit blocks a saturated provider while another provider runs", async () => {
    const manager = makeManager({ maxRunningJobs: 10, maxRunningJobsPerProvider: 1 });
    const sleep1 = await manager.startJob("sleep" as LlmCli, ["2"], "corr-s1");
    const sleep2 = await manager.startJob("sleep" as LlmCli, ["2"], "corr-s2");
    // Different provider (sh) still has capacity globally and per-provider.
    const sh1 = await manager.startJob("sh" as LlmCli, ["-c", "sleep 2"], "corr-sh1");

    expect((await manager.getJobSnapshot(sleep1.id)!).status).toBe("running");
    expect((await manager.getJobSnapshot(sleep2.id)!).status).toBe("queued");
    expect((await manager.getJobSnapshot(sh1.id)!).status).toBe("running");
    expect(manager.getLimiterSnapshot().saturated).toBe(true);

    await manager.cancelJob(sleep1.id);
    await manager.cancelJob(sleep2.id);
    await manager.cancelJob(sh1.id);
  });

  it("does not report saturated when capacity is available and no provider is queued", async () => {
    const manager = makeManager({ maxRunningJobs: 3, maxRunningJobsPerProvider: 2 });
    const job = await manager.startJob("sleep" as LlmCli, ["2"], "corr-a");
  ...
  it("rejects new work with a JobSaturationError when the queue is full", async () => {
    const manager = makeManager({
      maxRunningJobs: 1,
      maxRunningJobsPerProvider: 5,
      maxQueuedJobs: 1,
    });
    const a = await manager.startJob("sleep" as LlmCli, ["2"], "corr-a"); // running
    await manager.startJob("sleep" as LlmCli, ["2"], "corr-b"); // queued (fills queue)

    let thrown: unknown;
    try {
      await manager.startJob("sleep" as LlmCli, ["2"], "corr-c");
    } catch (err) {
      thrown = err;
    }
  ...
  it("cancels a queued job without ever spawning a process", async () => {
    const manager = makeManager({ maxRunningJobs: 1, maxRunningJobsPerProvider: 5 });
    const a = await manager.startJob("sleep" as LlmCli, ["2"], "corr-a"); // running
    const b = await manager.startJob("sleep" as LlmCli, ["2"], "corr-b"); // queued

    expect((await manager.getJobSnapshot(b.id)!).status).toBe("queued");
    const res = await manager.cancelJob(b.id);
    expect(res.canceled).toBe(true);
    expect((await manager.getJobSnapshot(b.id)!).status).toBe("canceled");
  ...
  it("releases the run slot when a running job completes so a queued one starts", async () => {
    ...
    await waitFor(
      async () =>
        (await manager.getJobSnapshot((await a).id)!).status === "completed" &&
        (await manager.getJobSnapshot((await b).id)!).status === "completed",
      6000
    );
    // Both slots released.
  ...
  it("acquireProcessSlot gates the direct-sync path under the same limiter", async () => {
    ...
    const a = await manager.startJob("sleep" as LlmCli, ["2"], "corr-a"); // holds the only slot
