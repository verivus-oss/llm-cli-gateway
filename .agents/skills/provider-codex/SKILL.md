...
- Tested against codex-cli 0.152.1. Codex advertises mcp-server / app-server transports, not a native ACP entrypoint, so `provider-acp://codex` reports `native:false` (no methods, no adapter-as-native masquerade) and `codex_request` exposes no `transport:"acp"` selector.
...
- `--output-schema`, `-c key=value`, `--ephemeral`, safety bypasses (on review), `--json` etc. are accepted on resume/review (codex-cli 0.152.1). Example (resume):
  ```
