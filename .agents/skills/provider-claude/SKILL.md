...
- Tested against claude 2.1.233. Claude Code is CLI-first with no native ACP entrypoint at this version; `provider-acp://claude` reports `native:false` (no methods, no adapter-as-native masquerade) and `claude_request` exposes no `transport:"acp"` selector.
