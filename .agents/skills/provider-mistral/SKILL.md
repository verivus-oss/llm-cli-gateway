...
- Tested against vibe 2.24.1. Vibe has a native ACP entrypoint (`vibe-acp`); `mistral_request` accepts `transport:"acp"` and `provider-acp://mistral` reports the negotiated capability set (fails closed unless `[acp]` and the provider `runtime_enabled` gate are set). ACP rejects `approvalStrategy:"mcp_managed"` and any `approvalPolicy`; the Mistral CLI rejects them as well because managed approval is Claude-only.
...
- Vibe 2.24.1 advertises repeatable `--disabled-tools`; the gateway maps
