...
- **`src/doctor.ts`** - The `doctor` CLI subcommand: probes provider CLIs, auth state, persistence config, and DB health; emits JSON validated against `setup/status.schema.json`. It reports no storage health: no DB file size, row counts, retention state, or wedged-run counts.
