...
- `.agents/skills/*/SKILL.md` - Agent-facing skills, all of which ship in the npm package and are exposed as `skills://` MCP resources: the workflow skills (async-job-orchestration, multi-llm-review, session-workflow, secure-orchestration, implement-review-fix, retrospective-walk, public-demo-session, least-cost-routing, personal-agent-config-kit), the per-provider skills under `.agents/skills/provider-*/`, and gateway-restart-surfaces. Every skill directory must be listed in `package.json#files`; `src/__tests__/skill-packaging.test.ts` fails the build otherwise, because a skill that is registered but unpackaged is invisible to every npm consumer.
