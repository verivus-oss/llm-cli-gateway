...
- **`src/personal-config.ts`** / **`src/personal-config-types.ts`** - Personal Agent Config Kit for a single developer's verified Git baseline and repository overlay across local workstations. Kit execution is local-only, requires durable SQLite or PostgreSQL job admission, supports Claude, Codex, and Mistral only (declared once as `personalConfigKit.supported` in `src/provider-definitions.ts`, which both admission gates and every read-only surface derive from), binds context and native continuity to the current process, and fails closed when provenance or recovery evidence is incomplete. `src/codex-kit-isolation.ts`, `src/mistral-kit-isolation.ts`, `src/mcp-artifact-admission.ts`, and `src/mcp-artifact-recovery.ts` enforce its provider-isolation and durable-artifact boundaries.
...
- `.agents/skills/*/SKILL.md` - Agent-facing skills, exposed as `skills://` MCP resources. Every git-tracked skill ships in the npm package: the workflow skills (async-job-orchestration, multi-llm-review, session-workflow, secure-orchestration, implement-review-fix, retrospective-walk, public-demo-session, least-cost-routing, personal-agent-config-kit) and the per-provider skills under `.agents/skills/provider-*/`. `gateway-restart-surfaces` is gitignored on purpose: it is host-local operational guidance, so it is neither committed nor shipped, and it appears only in a working tree that has it. Every tracked skill must be listed in `package.json#files`; `src/__tests__/skill-packaging.test.ts` fails the build otherwise, because a skill that is registered but unpackaged is invisible to every npm consumer. That ratchet compares against `git ls-files`, not the filesystem, since `npm pack` and a directory listing both see untracked files that a consumer never receives.
