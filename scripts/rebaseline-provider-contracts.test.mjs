...
describe("removals are never applied", () => {
  it("keeps reporting removals, because a version boundary is worth recording", () => {
    // Detection is the useful half. Applying it is what took capability from
    // customers, and run today the rebaseliner still nominates grok
    // --best-of-n, grok --check and devin --agent-config: exactly the three a
    // release once removed.
    const source = readFileSync(
      new URL("./rebaseline-provider-contracts.mjs", import.meta.url),
      "utf8"
    );
    expect(source).toMatch(/removedFlagDrift: plan\.removals/);
    expect(source).toMatch(/NOT removed/);
  });

  it("exports no writer that can remove a flag", async () => {
    // The fence is the deletion, not a flag on a code path that still exists.
    // Run today the rebaseliner nominates grok --best-of-n, grok --check and
    // devin --agent-config, which are exactly the three a release once removed.
    const module = await import("./rebaseline-provider-contracts.mjs");
    for (const name of Object.keys(module)) {
      expect(name, `${name} can still rewrite a removal`).not.toMatch(/^rewriteRemoved/);
    }
  });
});
