 ...
 *
 * Two rules, both about the same defect: SQL escaping the storage layer.
 *
 ...
 *
 * Tests are exempt from rule 2 by design: a test inspecting the database
 * directly is not a caller of the port. They are NOT exempt from rule 1.
 */
...
function sqlLiterals(body) {
  const found = [];
  for (const match of body.matchAll(TEMPLATE_LITERAL)) {
    if (SQL_SHAPES.some(shape => shape.test(match[0]))) {
...
for (const file of walk(SRC)) {
  ...
  // Tests are exempt from rule 1: seeding and inspecting a fixture database is
  // what a storage test IS, and the *-pg suites are engine-specific on purpose.
  // The port has to carry production code, not test scaffolding.
