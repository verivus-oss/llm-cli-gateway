...
import { findShippedFetchViolations } from "../shipped-fetch-policy.mjs";
...
export function classifyInstance(inst, ledger, baselineIndex) {
  ...
  const verAccepted =
    Array.isArray(entry.acceptedVersions) && entry.acceptedVersions.includes(inst.version);
...
export function classifyClosure(freshInstances, ledger, baseline, opts = {}) {
  ...
  const exit = Math.max(
    0,
    ...rows.map(r => r.exit),
    ...dropped.map(d => d.exit),
    ...invariants.map(i => i.exit)
  );
...
function freshInstances() {
  ...
  try {
    ...
    writeFileSync(
      ...
      readFileSync(join(REPO_ROOT, "package-lock.json"))
    );
 ...
 * release-security-audit.sh). Walks shipped dist JavaScript and declaration
...
export function fetchInDistFindings(repoRoot) {
  ...
  const walk = dir => {
    for (const ent of readdirSync(dir, { withFileTypes: true })) {
      // Mirror release-security-audit.sh scope exactly: skip __tests__ and
      // inspect only JavaScript and declaration files that can ship.
      if (ent.isDirectory()) {
        ...
        if (findShippedFetchViolations(relativePath, readFileSync(fullPath, "utf8")).length > 0) {
          findings.push({
            path: relativePath,
            name: "dist",
            version: "-",
...
function renderReportMd(result) {
  const lines = [
    `# supply-chain guard report`,
    ``,
    `verdict exit: ${result.exit}`,
    ``,
    `## counts`,
  ];
...
function main(argv) {
  ...
  let freshList;
  if (closureIdx !== -1)
    freshList = closureFromFixture(resolve(process.cwd(), argv[closureIdx + 1]));
  ...
  if (closureIdx === -1) {
    if (!existsSync(LICENSE_ALLOWLIST_PATH)) {
      process.stderr.write(
        `[supply-chain-guard] missing ${LICENSE_ALLOWLIST_PATH}; run --seed or restore it.\n`
      );
      return 1;
    }
    if (!existsSync(SOCKET_YML_PATH)) {
      process.stderr.write(
        `[supply-chain-guard] missing ${SOCKET_YML_PATH} (Socket policy source).\n`
      );
      return 1;
    }
    ...
    extraFindings.push(
      ...socketPolicyFindings(
        parseSocketIssueRules(readFileSync(SOCKET_YML_PATH, "utf8")),
  ...
  const outDir =
    outIdx !== -1
      ...
      : join(REPO_ROOT, ".supply-chain", `scan-${stamp}`);
  ...
  writeContractStubs(outDir, [
    ...result.rows.filter(r => r.exit > 0),
    ...result.dropped,
    ...result.invariants,
  ]);
  ...
  if (result.exit === 1)
    process.stderr.write(
      `[supply-chain-guard] ledger error:\n  ${result.ledgerErrors.join("\n  ")}\n`
    );
  else
    process.stderr.write(
      `[supply-chain-guard] verdict exit ${result.exit}; report at ${outDir}/report.md\n`
    );
