...
- **Codex** — `codex_request` and `codex_request_async` accept new
  ...
  `--add-dir <DIR>` (one instance per entry). (This entry's claim about resume inheriting the original session's cwd/policy was later found to be wrong; see the Codex resume correction under [Unreleased].)
