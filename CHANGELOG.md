...
- **Gemini** is not re-wired: `--include-directories` was wired in master
  ...
  changed. (This entry's claim that resume inherits the original session's cwd/policy was later found to be wrong; see the Codex resume correction under [Unreleased].)
...
- **`resolveCodexSessionArgs` helper** in `src/request-helpers.ts` with 7 new tests covering mode resolution and `gw-*` rejection (Codex uses an `exec resume` subcommand rather than a flag pair, so the helper returns a `mode` discriminant: `new` | `resume-by-id` | `resume-latest`) (This entry's claim that resume inherits the original session's cwd/policy was later found to be wrong; see the Codex resume correction under [Unreleased].)
